...
if generate_data_flag == true
    ...
    kkt_cond_list_original = [];
    kkt_cond_list_ruiz = [];
    kkt_cond_list_qr = [];
    kkt_cond_list_hyper = [];
    
    presolve_time_list_original = [];
    presolve_time_list_ruiz = [];
    presolve_time_list_qr = [];
    presolve_time_list_hyper = [];
    
    preconditioning_iters_list_original = [];
    preconditioning_iters_list_ruiz = [];
    preconditioning_iters_list_qr = [];
    preconditioning_iters_list_hyper = [];
    
    pipg_time_list_original = [];
    pipg_time_list_ruiz = [];
    pipg_time_list_qr = [];
    pipg_time_list_hyper = [];
    
    pipg_iters_list_original = [];
    pipg_iters_list_ruiz = [];
    pipg_iters_list_qr = [];
    pipg_iters_list_hyper = [];
    
    Qf_scale_list = [1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6];
    
    for j = 1:length(Qf_scale_list)
    
        Qf_scale = Qf_scale_list(j);
        ...
        % PIPG + Modified Ruiz Equilibration
        [P_ruiz, q_ruiz, H_ruiz, h_ruiz, c_ruiz, D_ruiz, E_ruiz, ruiz_iters, ruiz_time] = ruiz_mex(P, q, H, h, ps);
        [sigma_max_ruiz, ~, power_iteration_time_ruiz] = power_iteration_mex(H_ruiz, s);
        [z_ruiz, w_ruiz, iters_ruiz, pipg_time_ruiz] = pipg_mex(P_ruiz, q_ruiz, H_ruiz, h_ruiz, c_ruiz, D_ruiz, E_ruiz, sigma_max_ruiz, ground_truth, p, s);
        solve_time_pipg_ruiz = ruiz_time + power_iteration_time_ruiz + pipg_time_ruiz;
        ...
        pipg_iters_list_ruiz = [pipg_iters_list_ruiz, iters_ruiz];
    
        % PIPG + QR Preconditioning
        [P_qr, q_qr, H_qr, h_qr, c_qr, D_qr, E_qr, qr_time] = qr_preconditioner_mex(P, q, H, h);
        [sigma_max_qr, ~, power_iteration_time_qr] = power_iteration_dense_mex(H_qr, s);
        [z_qr, w_qr, iters_qr, pipg_time_qr] = pipg_dense_mex(P_qr, q_qr, H_qr, h_qr, c_qr, D_qr, E_qr, sigma_max_qr, ground_truth, p, s);
        solve_time_pipg_qr = qr_time + power_iteration_time_qr + pipg_time_qr;
        ...
        % PIPG + Hypersphere Preconditioning
        [P_hyper, q_hyper, H_hyper, h_hyper, c_hyper, D_hyper, E_hyper, sigma_max_hyper, shifted_power_iters, shifted_power_iteration_time, hypersphere_time] = hypersphere_mex(P, q, H, h, s, ps);
        [z_hyper, w_hyper, iters_hyper, pipg_time_hyper] = pipg_mex(P_hyper, q_hyper, H_hyper, h_hyper, c_hyper, D_hyper, E_hyper, sigma_max_hyper, ground_truth, p, s);
        solve_time_pipg_hyper = hypersphere_time + pipg_time_hyper;
        ...
        pipg_iters_list_hyper = [pipg_iters_list_hyper, iters_hyper];
...
if save_data_flag == true
    ...
    cond_data = table( ...
        Qf_scale_list.', ...
        kkt_cond_list_original.', kkt_cond_list_ruiz.', kkt_cond_list_qr.', kkt_cond_list_hyper.', ...
        'VariableNames', ...
        { ...
            'Qf_scale_list', ...
            'kkt_cond_list_original', 'kkt_cond_list_ruiz', 'kkt_cond_list_qr', 'kkt_cond_list_hyper', ...
    ...
    writetable(cond_data, 'data/cond_data.csv');

    solve_stats = table( ...
        Qf_scale_list.', ...
        presolve_time_list_original.', presolve_time_list_ruiz.', presolve_time_list_qr.', presolve_time_list_hyper.', ...
        preconditioning_iters_list_original.', preconditioning_iters_list_ruiz.', preconditioning_iters_list_qr.', preconditioning_iters_list_hyper.', ...
        pipg_time_list_original.', pipg_time_list_ruiz.', pipg_time_list_qr.', pipg_time_list_hyper.', ...
        pipg_iters_list_original.', pipg_iters_list_ruiz.', pipg_iters_list_qr.', pipg_iters_list_hyper.', ...
        'VariableNames', ...
        { ...
            'Qf_scale_list', ...
            'presolve_time_list_original', 'presolve_time_list_ruiz', 'presolve_time_list_qr', 'presolve_time_list_hyper', ...
    ...
    writetable(solve_stats, 'data/solve_stats.csv');
